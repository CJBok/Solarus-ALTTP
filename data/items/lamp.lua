...
function item:on_npc_collision_fire(npc)
  ...
  if npc:get_name():find("^torch") then
    ...
    if torch_sprite:get_animation() == "unlit" then
      ...
      if map:get_game().get_light ~= nil then
        -- Light the room.
        self.was_dark_room = true
        map:get_game():add_light(4)
      end
    end
  end
end
